...
class LongitudinalFCDenseNet(BaseModel):
    def __init__(
            ...
            out_chans_first_conv=48, n_classes=2, encoder=None, siamese=True
    ):
        super().__init__()
        ...
        if self.siamese:
            self.add_module(
                'merge_conv',
                nn.Conv2d(prev_block_channels * 2, prev_block_channels, 1, 1)
            )
    ...
    def forward(self, x_ref, x):
        if self.siamese:
            ...
            out, skip_connections = self.densenet_encoder(
                torch.cat((x_ref, x), dim=1)
            )

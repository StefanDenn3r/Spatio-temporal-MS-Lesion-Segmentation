...
class LongitudinalDeformationTrainer(ISBITrainer):
    """
    Trainer class
    """
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (input_moving, input_fixed, target_moving, target_fixed) in enumerate(self.data_loader):
            ...
            warp, flow = self.model(input_moving, input_fixed)

            # Calculate loss
            loss = self.loss(warp, flow, input_fixed)
            loss.backward()

            self.optimizer.step()

            self.writer.set_step(self.get_train_step(batch_idx, epoch))
            ...
            if not (batch_idx % self.log_step):
                ...
                log_visualizations_deformations(self.writer, input_moving, input_fixed, flow, target_moving, target_fixed)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (input_moving, input_fixed, target_moving, target_fixed) in enumerate(self.valid_data_loader):
                ...
                self.writer.set_step(self.get_val_step(batch_idx, epoch), 'valid')
